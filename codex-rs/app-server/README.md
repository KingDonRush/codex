...
- `model/list` — list available models (set `includeHidden: true` to include entries with `hidden: true`; pass `modelProvider` to inspect a configured provider other than the active one), with reasoning effort options, `additionalSpeedTiers`, `serviceTiers`, optional `defaultServiceTier`, optional legacy `upgrade` model ids, optional `upgradeInfo` metadata (`model`, `upgradeCopy`, `modelLink`, `migrationMarkdown`), and optional `availabilityNux` metadata.
- `modelProvider/capabilities/read` — read provider-level capabilities for the currently configured model provider, or for the configured provider named by `modelProvider`.
- `modelProvider/list` — list configured model providers with sanitized connection metadata, active-provider state, wire API, capability flags, and header/env-var names only; secret header values and bearer tokens are never returned.
