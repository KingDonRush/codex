...
`codex`, `codex-app-server`, and `claudex`. The `version` field in
`codex-package.json` is read from `[workspace.package].version` in
`codex-rs/Cargo.toml`. The `claudex` variant also includes the `codex`
binary as an internal companion because the Claudex entrypoint uses it as the
...
- `claudex` variant: the internal `codex` companion binary
...
binary instead of rebuilding it. For the `claudex` variant, the release output
directory must also contain the companion `codex` binary.
